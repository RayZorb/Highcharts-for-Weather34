...
class highcharts_temp_week(SearchList):
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        t2 = time.time()
        logdbg2("highcharts_temp_week SLE executed in %0.3f seconds" % (t2 - t1))

        # Return our json data
        return [self.search_list_extension]

class highcharts_bar_rain_week(SearchList):
    """SearchList to generate JSON vectors for Highcharts week plots."""
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        t2 = time.time()
        logdbg2("highcharts_bar_rain_week SLE executed in %0.3f seconds" % (t2 - t1))

        # Return our json data
        return [self.search_list_extension]
  
class highcharts_wind_week(SearchList):
    """SearchList to generate JSON vectors for Highcharts week plots."""
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        t2 = time.time()
        logdbg2("highcharts_wind_week SLE executed in %0.3f seconds" % (t2 - t1))

        # Return our json data
        return [self.search_list_extension]
        
class highcharts_solar_week(SearchList):
    """SearchList to generate JSON vectors for Highcharts week plots."""
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        t2 = time.time()
        logdbg2("highcharts_solar_week SLE executed in %0.3f seconds" % (t2 - t1))

        # Return our json data
        return [self.search_list_extension]
     
class highcharts_indoor_derived_week(SearchList):
    """SearchList to generate JSON vectors for Highcharts week plots."""
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        t2 = time.time()
        logdbg2("highcharts_indoor_derived_week SLE executed in %0.3f seconds" % (t2 - t1))

        # Return our json data
        return [self.search_list_extension]
     
class highcharts_year(SearchList):
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        if (self.numDays == 3):
                _start_ts = time.mktime((_ts_dt - datetime.timedelta(days=3)).timetuple())
                _end_ts = time.mktime((_ts_dt + datetime.timedelta(days=6)).timetuple())
                _timespan = TimeSpan(_start_ts, _end_ts)
        ...
        dewpointMinMax_json = json.dumps(zip(time_ms, dewpointMinRound, dewpointMaxRound))
