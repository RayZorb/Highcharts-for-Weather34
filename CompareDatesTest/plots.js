...
var jsonfileforplot = {
    temperatureplot: [['temp_week.json'],['year.json'],['temp_week1.json'],['year1.json']],
...
function addYearOptions(obj) {
    if (compare_dates) return  addWeekOptions(obj);
    obj.rangeSelector.buttons = [{
        type: 'day',
        count: 1,
        text: '1d'
...
function create_temperature_chart(options, span, seriesData, units){
     if (span[0] == "yearly"){
        if (compare_dates){
            options = create_chart_options(options, 'columnrange', 'Temperature Dewpoint Averages', '\xB0' + units.temp, [['Average Temperature','spline'],['Average Dewpoint', 'spline'],['Average Temperature','spline',,,,,1],['Average Dewpoint', 'spline',,,,,1]]);
            options.series[0].data = convert_temp(seriesData[0].temperatureplot.units, units.temp, reinflate_time(seriesData[0].temperatureplot.outTempaverage));
            options.series[1].data = convert_temp(seriesData[0].temperatureplot.units, units.temp, reinflate_time(seriesData[0].dewpointplot.dewpointaverage));
            create_compare_days_ts(options.series[0].data, seriesData[1].temperatureplot.outTempaverage);
            options.series[2].data = convert_temp(seriesData[1].temperatureplot.units, units.temp, reinflate_time(seriesData[1].temperatureplot.outTempaverage, options.series[0].data[0][0]));
            options.series[3].data = convert_temp(seriesData[1].temperatureplot.units, units.temp, reinflate_time(seriesData[1].dewpointplot.dewpointaverage, options.series[0].data[0][0]));
        }else{
...
function display_chart(units, plot_type, span, dplots = false, cdates = false, reload_plot_type_span = null, realtime = false){
    ...
    for (var i = 0; i < (span=='weekly'?jsonfileforplot[plot_type][2].length:jsonfileforplot[plot_type][3].length); i++,index++)
        if (compare_dates && (span=='weekly'?jsonfileforplot[plot_type][2]:jsonfileforplot[plot_type][3]) != null)
            files[index] = pathjsondayfiles + (span=='weekly'?jsonfileforplot[plot_type][2][i]:jsonfileforplot[plot_type][3][i]);
    if (buttons == null){
        ...
        function compare_callback(){return function(){
                                    ...
                                    window.location.href= dayplotsurl+"?units="+units.temp+","+units.pressure+","+units.wind+","+units.rain+"&plot_type="+plot_type+","+pathjsondayfiles+(span=='weekly'?jsonfileforplot[plot_type][0]:jsonfileforplot[plot_type][1])+","+weereportcmd+","+pathjsondayfiles+(span=='weekly'?jsonfileforplot[plot_type][2]:jsonfileforplot[plot_type][3])+","+reload_plot_type+":"+reload_span+",false"+","+span+"&weewxpathbin="+pathweewxbin+"&epoch="+epoch+"&epoch1="+epoch1}};
