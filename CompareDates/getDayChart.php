...
<?php 
    ...
    if (sizeof($plot_info) == 5){
      ...
      for($i = 1; $i < 4; $i++){
        ...
        if (file_exists($plot_info[1])) {
          echo "<script> display_chart({temp:"."'".$units[0]."',pressure:"."'".$units[1]."',wind:"."'".$units[2]."',rain:"."'".$units[3]."'},'".$plot_info[0]."','weekly',false,true,'".$plot_info[4]."');</script>";
          return;
        }
      }
    }
      ...
      for($i = 2; $i > 0; $i--){
        ...
        $output = shell_exec(escapeshellcmd($plot_info[2]." ".(time()<$day_epoch?0:$day_epoch)." ".$plot_info[1].".tmpl ".getcwd()));
        if (file_exists($plot_info[1])) {
          echo "<script> display_chart({temp:"."'".$units[0]."',pressure:"."'".$units[1]."',wind:"."'".$units[2]."',rain:"."'".$units[3]."'},'".$plot_info[0]."','weekly',true,false,'".$plot_info[3]."');</script>";
          return;
        }
      }
    }
